...
UPDATE wbsc.passwords SET creation_date = to_date('01-01-2099', 'DD-MM-YYYY') WHERE passwordid = (select passwordid from wbsc.passwords where userid = (select userid from wbsc.sc_users where surname='ipccimporterdaemon' and login='ipccimporterdaemon') order by creation_date desc limit 1);
